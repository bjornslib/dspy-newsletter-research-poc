#!/usr/bin/env node
/**
 * Sync Task Master tasks.json → feature_list.json
 *
 ...
 *
 * Usage: node .claude/scripts/sync-taskmaster-to-features.js [--dry-run] [--output=path]
 */
...
function extractValidation(task) {
  ...
  if (combined.includes('playwright') || combined.includes('e2e') || combined.includes('browsermcp')) {
    return {
      type: 'browser',
...
function main() {
  ...
  if (isDryRun) {
    ...
    console.log('   1. Review: git diff .claude/state/feature_list.json');
    console.log('   2. Validate: cat .claude/state/feature_list.json | jq "."');
